...
def test_text_preprocessor():
    ...
    for input_text, expected in test_cases:
        ...
        if result != expected:
            all_passed = False
        logger.debug(f"{status} preprocess('{input_text}') = '{result}' (期望: '{expected}')")

    logger.info(f"预处理测试: {'全部通过' if all_passed else '有失败'}")
    return all_passed
...
def test_config_manager():
    ...
    logger.debug(f"配置加载状态: {config.is_loaded()}")
    ...
    for text, expected in exact_tests:
        ...
        if result != expected:
            all_passed = False
        logger.debug(f"{status} get_exact_match('{text}') = '{result}' (期望: '{expected}')")
    ...
    for text, expected in prefix_tests:
        ...
        if result != expected:
            all_passed = False
        logger.debug(f"{status} match_prefix('{text}') = '{result}' (期望: '{expected}')")

    logger.info(f"配置管理测试: {'全部通过' if all_passed else '有失败'}")
    return all_passed
...
def test_text_filter():
    ...
    for text, expected_output in prefix_tests:
        ...
        if result.result_type != FilterResultType.PREFIX_MATCH or result.output_text != expected_output:
            all_passed = False
        logger.debug(f"{status} filter('{text}') -> {result.result_type.value}, output='{result.output_text}'")
        logger.debug(f"    预处理结果: '{result.processed_text}'")

    # 测试精确匹配
    logger.info("测试精确匹配过滤:")
    exact_tests = [
        ("ID", "标识符"),
        ("NO", "编号"),
        ("name", "名称"),
    ...
    for text in model_tests:
        ...
        if result.result_type != FilterResultType.NEED_MODEL:
            all_passed = False
        logger.debug(f"{status} filter('{text}') -> {result.result_type.value}")
        logger.debug(f"    预处理结果: '{result.processed_text}'")

    logger.info(f"文本过滤测试: {'全部通过' if all_passed else '有失败'}")
    return all_passed
...
def test_integration():
    ...
    for case in test_cases:
        ...
        logger.debug(f"{status} {case['description']}")
        logger.debug(f"    输入: '{case['input']}'")
        logger.debug(f"    预处理: '{result.processed_text}'")
        logger.debug(f"    结果类型: {result.result_type.value}")
        logger.debug(f"    输出: '{result.output_text}'")
        logger.debug(f"    置信度: {result.confidence}")

    logger.info(f"集成测试: {'全部通过' if all_passed else '有失败'}")
    return all_passed
